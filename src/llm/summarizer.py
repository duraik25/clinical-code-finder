...
class ResultSummarizer:
    ...
    async def summarize1(
        ...
        for system, items in results.items():
            if items:
                results_summary.append(f"{system}: {len(items)} codes found")
                for item in items[:2]:
                    results_summary.append(f"- {item['code']}: {item['display']}")

        results_text = "\n".join(results_summary) if results_summary else "No codes found"
